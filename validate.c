...
int validateUsername(char* username) {
	...
	if(!inValidRange(length, MIN_LENGTH_OF_USERNAME, MAX_LENGTH_OF_USERNAME)) {
        printf("User must have length from 3 to 20");
        return 0;
	}
    if(isHasBlankSpace(username)) {
	    printf("Do not use blank space in username\n");     
        return 0;
	}
	return 1;
}
...
int validatePassword(char* password) {
	...
	if(!inValidRange(length, MIN_LENGTH_OF_USERNAME, MAX_LENGTH_OF_USERNAME)) {
        printf("Password must have length from 3 to 20");
        return 0;
	}
    if(isHasBlankSpace(password)) {
	    printf("Do not use blank space in password\n");     
        return 0;
	}
	return 1;
}
